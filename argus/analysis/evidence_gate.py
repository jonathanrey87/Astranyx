    ...
    def evaluate(self, evidence: FindingEvidence) -> EvidenceDecision:
        ...
        if category in {"pii", "pii_disclosure"}:
            if evidence.sensitive_value.strip():
                return EvidenceDecision(True, "Non-empty sensitive value observed")
            return EvidenceDecision(False, "No sensitive value was disclosed")

        if category in {"crlf", "header_injection"}:
            if evidence.injected_header:
                return EvidenceDecision(True, "A separate response header was injected")
            return EvidenceDecision(False, "Input was escaped or reflected only as text")
        ...
        if category == "open_redirect":
            ...
            if host and not self._trusted(host, evidence.trusted_hosts):
                return EvidenceDecision(True, "Redirect left the trusted domain")
            return EvidenceDecision(False, "Redirect remained on a trusted domain")
        ...
        if category in {"http_500", "server_error"}:
            ...
            if impact:
                return EvidenceDecision(True, "Server error had security impact")
            return EvidenceDecision(False, "Generic server error without impact")

        if category == "graphql":
            if evidence.protected_data:
                return EvidenceDecision(True, "Unauthorized protected data returned")
            return EvidenceDecision(False, "Query execution returned no protected data")

        if category in {"health", "health_endpoint"}:
            if evidence.data_exposure or evidence.sensitive_value.strip():
                return EvidenceDecision(True, "Health endpoint exposed sensitive data")
            return EvidenceDecision(False, "Health endpoint exposed status only")

        return EvidenceDecision(False, "Manual validation required")

    @staticmethod
    def _trusted(host: str, trusted_hosts: tuple[str, ...]) -> bool:
